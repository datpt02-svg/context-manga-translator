...
pub use koharu_core::UnlimitedOcrMode;
...
pub async fn run(
    ...
    'pages: for (page_index, page_id) in pages.iter().enumerate() {
        ...
        // ---- Smart Fallback: after all OCR engines, send uncertain boxes to Unlimited-OCR ----
        if spec.options.unlimited_ocr_mode == UnlimitedOcrMode::SmartFallback {
            let fallback_url = spec
                .options
                .unlimited_ocr_url
                .as_deref()
                .unwrap_or("http://127.0.0.1:7862");

            let scene_snap = session.scene_snapshot();
            let fallback_ctx = unlimited_ocr_fallback::FallbackCtx {
                scene: &scene_snap,
                page: *page_id,
                blobs: &session.blobs,
            ...
            if !fallback_ops.is_empty() {
                let batch = Op::Batch {
                    ops: fallback_ops,
                    label: format!("unlimited-ocr-fallback: page {}", page_id),
                };
                if let Err(err) = session.apply(batch) {
                    tracing::warn!("SmartFallback apply failed on page {}: {err:#}", page_id);
                    warning_count += 1;
                }
            }
        }
