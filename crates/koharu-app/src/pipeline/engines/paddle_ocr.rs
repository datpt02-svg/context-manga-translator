...
impl Engine for Model {
    async fn run(&self, ctx: EngineCtx<'_>) -> Result<Vec<Op>> {
        ...
        for ((node_id, tf, td), out) in texts.iter().zip(outputs) {
            let report = assess_ocr_quality(OcrQualityInput {
                text: Some(&out.text),
                detector_confidence: td.confidence,
                ocr_confidence: Some(td.confidence),
                bbox_width: tf.width,
            ...
            ops.push(Op::UpdateNode {
                ...
                patch: NodePatch {
                    data: Some(NodeDataPatch::Text(TextDataPatch {
                        ...
                        ocr_engine: Some(Some("paddle-ocr-vl-1.6".to_string())),
                        ocr_confidence: Some(Some(td.confidence)),
