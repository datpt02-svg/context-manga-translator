...
impl Model {
    ...
    /// Send a raw prompt through the LLM and return the raw response.
    /// No tagged-block formatting or response parsing — pure prompt in, text out.
    /// Used by the rich-context translation path.
    pub async fn translate_raw(
        ...
        let result = match &mut *guard {
            State::ReadyLocal(llm) => {
                let opts = llm.id().default_generate_options();
                llm.generate(prompt, &opts, target_language, system_prompt)
            }
            State::ReadyProvider { target, provider } => {
                provider
                    .translate(prompt, target_language, &target.model_id, system_prompt)
                    .await
            }
            State::Loading { .. } => Err(anyhow::anyhow!("LLM is still loading")),
            State::Failed { error, .. } => Err(anyhow::anyhow!("LLM failed to load: {error}")),
        ...
        Ok(strip_thinking_block(&result).to_string())
    }
}
